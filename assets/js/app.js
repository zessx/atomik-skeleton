$(document).ready(function(){
	...
	*/
	...
	/*
	* Add a confirm button to all a.btn-modal
	*/
	$('a.btn-modal').on('click', function(ev) {
		ev.preventDefault();
		var modalTitle 			= $(this).data('modalTitle') ? $(this).data('modalTitle') : 'Confirmation de la suppression';
		var modalContent 		= $(this).data('modalContent') ? $(this).data('modalContent') : 'Êtes-vous sûr de vouloir supprimer cet élément ?';
		var modalConfirm 		= $(this).data('modalConfirm') ? $(this).data('modalConfirm') : 'Oui, supprimer';
		var modalConfirmClass 	= $(this).data('modalConfirmClass') ? $(this).data('modalConfirmClass') : 'danger';
		var modalCancel			= $(this).data('modalCancel') ? $(this).data('modalCancel') : 'Non, annuler';
		var modalCancelClass	= $(this).data('modalCancelClass') ? $(this).data('modalCancelClass') : 'default';
		var modal = $('<div class="modal fade"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">'+modalTitle+'</h4></div><div class="modal-body"><p>'+modalContent+'</p></div><div class="modal-footer"><a type="button" class="btn btn-'+modalCancelClass+'" data-dismiss="modal">'+modalCancel+'</a><a type="button" class="btn btn-'+modalConfirmClass+'" href="'+$(this).attr('href')+'">'+modalConfirm+'</button></div></div></div>');
		$('body').append(modal);
		modal.modal('show');
	});
